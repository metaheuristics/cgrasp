...
double CECShiftedSphere::getGap(){
	return (bestValue - minValue);
}

bool CECShiftedSphere::isNearOptimum(double fBest){
...
double CECShiftedSphere::calc(double *x){
	cont++;  
	long double value,sum=0;
	int i;

	for (i = 0; i < n; i++){
		sum += pow(x[i]-o[i],(long double) 2);
	}
...
double CECShiftedSphere::calc2(ap::real_1d_array x){
	cont++;  
	long double value,sum=0;
	int i;

	for (i = 1; i <= n; i++){
		sum += pow(x(i)-o[i-1],(long double) 2);
	}
