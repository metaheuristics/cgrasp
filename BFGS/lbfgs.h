...
-----------------------------------------------*/
//void funcgrad(double *x, double& f, double *g);
...
class LBFGS{
	...
	private:
		...
		double dotproduct(const int& n, const ap::real_1d_array& dx, int sx, 
				const ap::real_1d_array& dy, int sy);

		void lincomb(const int& n, const double& da, const ap::real_1d_array& dx,
	 			int sx, ap::real_1d_array& dy, int sy);

		void mcstep(double& stx, double& fx, double& dx, double& sty, double& fy,
     			double& dy, double& stp, const double& fp, const double& dp, bool& brackt,
     			const double& stmin, const double& stmax, int& info);

		void newiteration(const ap::real_1d_array& x, double f, const ap::real_1d_array& g);

		void mcsrch(const int& n, ap::real_1d_array& x, double& f, ap::real_1d_array& g, 			    const ap::real_1d_array& s, int sstart, double& stp, const double& ftol,				const double& xtol, const int& maxfev, int& info, int& nfev, ap::real_1d_array& wa, 
				const double& gtol, const double& stpmin, const double& stpmax);
				
	public:
		LBFGS(Funcao *func, bool converge);
		virtual ~LBFGS();

		void minimize(int n, int m, ap::real_1d_array& x, double epsg, double epsf, 
				double epsx, int maxits, int& info);
