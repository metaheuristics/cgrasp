...
real CECShiftedSchwefelNoise::getGap(){
	return (bestValue - minValue);
}

bool CECShiftedSchwefelNoise::isNearOptimum(real fBest){
...
real CECShiftedSchwefelNoise::calc(real *x){
	cont++;  
	real z, noise;	
	real value,sum=0;
	int i, j;
...
real CECShiftedSchwefelNoise::calc2(ap::real_1d_array x){
	return -1.0;
}
...
real CECShiftedSchwefelNoise::randomperc(){
    return (sprng());
	//return Util::dRand();
}
...
// Retirado do arquivo rand.c do codigo do CEC
// Compute the noise 
real CECShiftedSchwefelNoise::randomnormaldeviate(){
    real t;
    if(rndcalcflag)
    {
