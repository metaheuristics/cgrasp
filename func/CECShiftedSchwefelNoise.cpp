...
CECShiftedSchwefelNoise::CECShiftedSchwefelNoise(int n){
	...
	// Inicia os parametros do gerador de ruidos retirado do codigo do CEC
	randomize();
	rndcalcflag = 1;
}

CECShiftedSchwefelNoise::~CECShiftedSchwefelNoise(){
	//delete []o;
}

void CECShiftedSchwefelNoise::setFnEvals(int c){
	cont = c;	
}

int CECShiftedSchwefelNoise::getFnEvals(){
	return cont;	
}

double CECShiftedSchwefelNoise::getGap(){
	return (bestValue - minValue);
}

bool CECShiftedSchwefelNoise::isNearOptimum(double fBest){
...
double CECShiftedSchwefelNoise::calc(double *x){
	cont++;  
	long double z, noise;	
	...
	// Multiplicar pelo ruido
	noise = 1.0 + 0.4*fabs(randomnormaldeviate()); 

	value = sum*noise - 450.0;
...
double CECShiftedSchwefelNoise::calc2(ap::real_1d_array x){
	return -1.0;
}


void CECShiftedSchwefelNoise::calcGrad(ap::real_1d_array &x, ap::real_1d_array &g){

}

// Retirado do arquivo rand.c do codigo do CEC
// Get seed number for random and start it up
void CECShiftedSchwefelNoise::randomize(){
...
long double CECShiftedSchwefelNoise::randomperc(){
    return (sprng());
	//return Util::dRand();
}
...
// Retirado do arquivo rand.c do codigo do CEC
// Compute the noise 
long double CECShiftedSchwefelNoise::randomnormaldeviate(){
    long double t;
    if(rndcalcflag)
    {
