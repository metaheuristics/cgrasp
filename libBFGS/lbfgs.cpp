...
LBFGS::LBFGS(Funcao *func, bool converge){
	this->func = func;
     ...
     int& info)
{
    ...
    while(true)
    {
        ...
        if( iter>maxits && maxits > 0 && !converge)
        {
            info = 5;
            return;
        } 
		...
		gnorm = sqrt(dotproduct(n, g, 1, g, 1));
        if( gnorm<=epsg && !converge)
        {
            info = 4;
            return;
        }
        tf = ap::maxreal(fabs(fold), ap::maxreal(fabs(f), 1.0));
		if( (fold-f<=epsf*tf) && !converge)
        {
            info = 1;
            return;
        }
        ...
        txnorm = ap::maxreal(xnorm, sqrt(dotproduct(n, xold, 1, xold, 1)));
		...
		if (converge && func->isNearOptimum(f)){
			info = 6;
			printf("Convergiu no BFGS... \n");
			return;
		}
...
void LBFGS::lincomb(const int& n,
     const double& da,
     const ap::real_1d_array& dx,
	 int sx,
     ap::real_1d_array& dy,
...
/*double lbfgsdotproduct(const int& n,
     ...
     int sy)
*/
double LBFGS::dotproduct(const int& n,     const ap::real_1d_array& dx,     int sx,     const ap::real_1d_array& dy,     int sy)
{
    double result;
    double v;
    int fx;
...
void LBFGS::mcsrch(const int& n,     ap::real_1d_array& x,     double& f,     ap::real_1d_array& g,     const ap::real_1d_array& s,     int sstart,     double& stp,     const double& ftol,     const double& xtol,     const int& maxfev,     int& info,     int& nfev,     ap::real_1d_array& wa,     const double& gtol,     const double& stpmin,     const double& stpmax)
{
    ...
    {
		printf("n<=0||stp<=0||ftol<0||gtol<zero||xtol<zero||stpmin<zero||stpmax<stpmin||maxfev<=0 \n");	    
		return;
    }
    dginit = 0;
    for(j = 1; j <= n; j++)
...
*/
void LBFGS::newiteration(const ap::real_1d_array& x,     double f,     const ap::real_1d_array& g)
{
//	printf("\tBFGS - Fx = %lf \n", f);

/*	int i = 0;

//	printf("Nova iteraçao... %d\n", cont);
